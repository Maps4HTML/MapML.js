...
test.describe("web-map DOM API Tests", () => {
  ...
  test("Creating map with the default size hides the fullscreen control and the controls attribute functions properly", async () => {
    ...
    await page.evaluate( viewer => viewer.removeAttribute("controls"), mapHandle);
    ...
    await page.evaluate( viewer => viewer.setAttribute("controls",""), mapHandle);
    zoomHidden = await page.$eval(".leaflet-top.leaflet-left > .leaflet-control-zoom", (div) => div.hidden);
    reloadHidden = await page.$eval(".leaflet-top.leaflet-left > .mapml-reload-button", (div) => div.hidden);
    expect(zoomHidden).toEqual(false);
    expect(reloadHidden).toEqual(false);
  ...
  test.describe("controlslist test", () => {
    ...
    test("Adding controlslist using setAttribute and controlsList setter", async () => {
      const mapHandle = await page.evaluateHandle(() => document.querySelector('map'));
      await page.evaluate( map => map.setAttribute("controlslist","noreload"), mapHandle);
      ...
      hascontrolslist = await page.evaluate( map => map.getAttribute("controlslist"), mapHandle);
    ...
    test("controlslist removeAttribute", async () => {
      const mapHandle = await page.evaluateHandle(() => document.querySelector('map'));
      // removeAttribute
      await page.evaluate( viewer => viewer.removeAttribute("controlslist"), mapHandle);
    
      let hasControlslist = await page.evaluate( viewer => viewer.hasAttribute("controlslist"), mapHandle);
