describe("mapml-viewer DOM API Tests", () => {
  ...
  test("Create a layer with document.createElement(layer-)", async () => {
    ...
    await expect(layerControlHidden).toEqual(false);
    ...
    await expect(layerVisible).toBe(true);
  ...
  test("Toggle all mapml-viewer controls by adding or removing controls attribute", async () => {
    ...
    await expect(hasControls).toBe(true);
    ...
    await expect(hasControls).toBe(false);
    ...
    await expect(mapControlsHidden).toBe(true);
