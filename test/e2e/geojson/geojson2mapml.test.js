...
test.describe('GeoJSON API - geojson2mapml', () => {
  ...
  test('Point Geometry (string json)', async () => {
    const out = await page.$$eval('layer-', (node) => node[0].outerHTML);
  ...
  test('Polygon Geometry', async () => {
    const out = await page.$$eval('layer-', (node) => node[2].outerHTML);
    const exp = await page.$eval(
  ...
  test('MultiPoint Geometry', async () => {
    const out = await page.$$eval('layer-', (node) => node[3].outerHTML);
    const exp = await page.$eval(
  ...
  test('MultiLineString Geometry', async () => {
    const out = await page.$$eval('layer-', (node) => node[4].outerHTML);
    const exp = await page.$eval(
  ...
  test('MultiPolygon Geometry', async () => {
    const out = await page.$$eval('layer-', (node) => node[5].outerHTML);
    const exp = await page.$eval(
  ...
  test('Geometry Collection', async () => {
    const out = await page.$$eval('layer-', (node) => node[6].outerHTML);
    const exp = await page.$eval(
  ...
  test('Feature Collection', async () => {
    const out = await page.$$eval('layer-', (node) => node[7].outerHTML);
    const exp = await page.$eval(
  ...
  test('BBOX, Options label, caption + properties string', async () => {
    const out = await page.$$eval('layer-', (node) => node[8].outerHTML);
    const exp = await page.$eval(
  ...
  test('BBOX, Options label, caption + properties function', async () => {
    const out = await page.$$eval('layer-', (node) => node[9].outerHTML);
    const exp = await page.$eval(
  ...
  test('Feature', async () => {
    const out = await page.$$eval('layer-', (node) => node[10].outerHTML);
    const exp = await page.$eval(
