...
test("propertyFunction + bbox test", async () => {
  const out8 = await page.$eval(
    "body > p#json8",
    (p) => JSON.parse(p.innerHTML)
  );
  const exp8 = await page.$eval(
     "body > p#exp8",
     (p) => JSON.parse(p.innerHTML)
   );
...
test("transform is false", async () => {
  const out9 = await page.$eval(
    "body > p#json9",
    (p) => JSON.parse(p.innerHTML)
  );
  const exp9 = await page.$eval(
     "body > p#exp9",
     (p) => JSON.parse(p.innerHTML)
   );
