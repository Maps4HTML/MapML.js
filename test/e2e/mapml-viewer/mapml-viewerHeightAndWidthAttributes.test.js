...
test.describe("Adding Width and Height Attribute to mapml-viewer", () => {
  ...
  test("Setting Width and Height Attibute to mapml-viewer", async () => {
    ...
    let height_with_px = await page.$eval("body > mapml-viewer", (viewer) => viewer._container.style.height);
    let width_with_px = await page.$eval("body > mapml-viewer", (viewer) => viewer._container.style.width);
  ...
  test("Only Width Added to A Map With No Width OR Height Attributes", async () => {
    ...
    let has_height = await page.$eval("body > mapml-viewer", (viewer) => viewer.hasAttribute("height"));
    expect(has_height).toEqual(false);
    //set a height attribute to a map with no height or width attributes
    await page.$eval("body > mapml-viewer", (viewer) => viewer.setAttribute("height","500"));
    //expected height to be true
    has_height = await page.$eval("body > mapml-viewer", (viewer) => viewer.hasAttribute("height"));
    expect(has_height).toEqual(true);
  });

  test("Only Height Added to A Map With No Width OR Height Attributes", async () => {
    await page.goto("noWidthAndHeight.html");
    let has_width = await page.$eval("body > mapml-viewer", (viewer) => viewer.hasAttribute("width"));
    expect(has_width).toEqual(false);
    //set a height attribute to a map with no height or width attributes
    await page.$eval("body > mapml-viewer", (viewer) => viewer.setAttribute("width","500"));
    //expected height to be true
    has_width = await page.$eval("body > mapml-viewer", (viewer) => viewer.hasAttribute("width"));
