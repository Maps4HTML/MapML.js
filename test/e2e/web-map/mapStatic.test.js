...
test.describe("Adding Static Attribute to map", () => {
  ...
  test("Setting New Static Attribute to map", async () => {
    ...
    await page.$eval("body > map", (viewer) => viewer.setAttribute("static",""));
    //panning, zooming, etc. disabled
    let drag = await page.$eval("body > map", (viewer) => viewer._map.dragging._enabled);
    let touchZoom = await page.$eval("body > map", (viewer) => viewer._map.touchZoom._enabled);
    let doubleClickZoom = await page.$eval("body > map", (viewer) => viewer._map.doubleClickZoom._enabled);
    let scrollWheelZoom = await page.$eval("body > map", (viewer) => viewer._map.scrollWheelZoom._enabled);
    let boxZoom = await page.$eval("body > map", (viewer) => viewer._map.boxZoom._enabled);
    let keyboard = await page.$eval("body > map", (viewer) => viewer._map.keyboard._enabled);
    let zoomControl = await page.$eval("body > map", (viewer) => viewer._zoomControl._disabled);
  ...
  test("Removing Static Attribute", async () => {
    ...
    await page.$eval("body > map", (viewer) => viewer.removeAttribute("static"));
