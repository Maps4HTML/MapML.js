...
test.describe("Adding Width and Height Attribute to map", () => {
  ...
  test("Setting New Width and Height Attributes to map", async () => {
    ...
    await page.$eval("body > map", (viewer) => viewer.setAttribute("width","800"));
    await page.$eval("body > map", (viewer) => viewer.setAttribute("height","800"));
    //actual height and width value of the map
    let height_without_px = await page.$eval("body > map", (viewer) => viewer.height);
    let width_without_px = await page.$eval("body > map", (viewer) => viewer.width);
  ...
  test("Map Property Dimension Match On Window Size Change", async () => {
    ...
    //actual height and width value of the map
    let _height = await page.$eval("body > map", (viewer) => window.getComputedStyle(viewer).height);
    let _width = await page.$eval("body > map", (viewer) => window.getComputedStyle(viewer).width);
  ...
  test("Only Width Added to A Map With No Width OR Height Attributes", async () => {
    ...
    let has_height = await page.$eval("body > map", (viewer) => viewer.hasAttribute("height"));
    expect(has_height).toEqual(false);
    //set a height attribute to a map with no height or width attributes
    await page.$eval("body > map", (viewer) => viewer.setAttribute("height","500"));
    //expected height to be true
    has_height = await page.$eval("body > map", (viewer) => viewer.hasAttribute("height"));
    expect(has_height).toEqual(true);
  });

  test("Only Height Added to A Map With No Width OR Height Attributes", async () => {
    await page.goto("mapNoWidthAndHeight.html");
    let has_width = await page.$eval("body > map", (viewer) => viewer.hasAttribute("width"));
    expect(has_width).toEqual(false);
    //set a height attribute to a map with no height or width attributes
    await page.$eval("body > map", (viewer) => viewer.setAttribute("width","500"));
    //expected height to be true
    has_width = await page.$eval("body > map", (viewer) => viewer.hasAttribute("width"));
